...
export function useMonthSummary(year: number, month: number) {
  ...
  return useMemo(() => {
    ...
    const activeTxns = transactions.filter(t => !t.isDeleted && t.date.startsWith(monthStr));
  ...
  return useMemo(() => {
    return transactions
      .filter(t => {
        ...
        if (filter.search && !t.note.toLowerCase().includes(filter.search.toLowerCase()))
          return false;
        return true;
      })
...
export function useBudgetSummary(year: number, month: number) {
  ...
  return useMemo(() => {
    ...
    const monthTxns = transactions.filter(t => !t.isDeleted && t.date.startsWith(period));
    ...
    for (const cat of categories.filter(c => c.isActive && !c.isDeleted && c.group !== 'Income')) {
      const budget = budgets.find(
        b => b.categoryId === cat.id && b.period === period && !b.isDeleted
      );
