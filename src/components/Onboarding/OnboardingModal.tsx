...
export default function OnboardingModal() {
  ...
  const [editableAccounts, setEditableAccounts] = useState<Partial<Account>[]>(defaults.accounts as any);
    ...
    <Dialog open={true} onOpenChange={() => {}}>
      <DialogContent className="max-w-xl sm:max-w-2xl overflow-hidden p-0 border-none shadow-2xl [&>button]:hidden">
        <div className="bg-primary/5 p-8 border-b border-border/50 text-center">
          ...
          <DialogTitle className="text-2xl font-bold tracking-tight mb-2">Welcome to Moniq</DialogTitle>
          <DialogDescription className="text-base text-muted-foreground">
            Let's get your ledger ready. Start with standard categories and accounts, or begin from a completely blank slate.
          </DialogDescription>
        </div>
        ...
        <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">Rename these starting accounts to match your actual banks and wallets. A corresponding Payment Method will be created for each automatically.</p>
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">Moniq will generate <b>{defaults.categories.length}</b> essential categories across Needs, Wants, and Income to save you time. You can customize these later in Settings.</p>
        ...
        <div className="p-6 bg-accent/20 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
          <Button variant="ghost" className="text-muted-foreground font-bold tracking-widest uppercase text-[10px]" onClick={handleStartFromScratch}>
            Skip & Start from Scratch
          </Button>
          <Button className="gap-2 px-8 font-bold tracking-widest uppercase text-[10px]" onClick={handleStartWithDefaults}>
